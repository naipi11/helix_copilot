...
impl MappableCommand {
    ...
    static_commands!(
        ...
        ghost_text_accept, "Accept ghost text inline completion",
...
fn normal_mode(cx: &mut Context) {
    // Clear inline completion ghost text when leaving insert mode
    let view = cx.editor.tree.get(cx.editor.tree.focus);
    if let Some(doc) = cx.editor.documents.get_mut(&view.doc) {
